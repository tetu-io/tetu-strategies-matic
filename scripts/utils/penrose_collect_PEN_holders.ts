...
import {
  ...
  ISmartVault__factory, IUserProxy__factory,
...
import {IClaimable} from "../../typechain/IClaimable";
...
async function main() {
  // const gov = await DeployerUtilsLocal.impersonate('0xcc16d636dD05b52FF1D8B9CE09B09BC62b11412B');
  ...
  for (const holder of holders.keys()) {
    ...
    for (let i = 0; i < votesLength.toNumber(); i++) {
      ...
      votes += `${name}(${vote.poolAddress}) = ${formatUnits(vote.weight)} |||`;
    }


    console.log(holder, holders.get(holder)?.toFixed(0));
    appendFileSync('tmp/holders.txt', `${owner};${holders.get(holder)?.toFixed(0)};${votes}\n`, 'utf8');
  }
