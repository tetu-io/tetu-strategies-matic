...
describe('Convex USDR am3CRV tests', async () => {
  if (argv.disableStrategyTests || argv.hardhatChainId !== 137) {
    return;
  }
  const underlying = MaticAddresses.CURVE_USDR_am3CRV_TOKEN;
  const strategyName = 'ConvexStrategyUSDRam3CRV';
  const tokenName = 'CONVEX_USDR_3CRV';
  ...
  before(async function () {
    await StrategyTestUtils.deployCoreAndInit(deployInfo, argv.deployCoreContracts);
      ...
      if (deployInfo.core) {
          ...
          console.log('!!!!! start getPriceWithDefaultOutput ', calculator.address);
          let result = await calculator.getPriceWithDefaultOutput('0xa138341185a9D0429B0021A11FB717B225e13e1F');
